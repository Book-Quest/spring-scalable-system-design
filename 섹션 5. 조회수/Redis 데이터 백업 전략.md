...
- fork를 떠서 자식 프로세스에서 백업을 하므로 본 프로세스 성능에는 영향이 없다.

**단점**

- 스냅샷을 추출하는데 시간이 오래걸린다.
- 스냅샷 저장 시점 사이의 데이터 변경 사항은 유실될 수 있으며, 저장 도중 서버가 꺼지면 이후 데이터가 모두 사라진다.
- fork를 이용하므로 시간이 오래결릴 수 있고, CPU와 메모리 자원을 소모한다.
...
- 특정 시점에 데이터 전체를 다시 쓰는 rewrite 기능을 제공한다.
    - 기존의 `appendonly.aof` 파일을 최적화된 새로운 파일로 교체하는 과정
...
3. write가 완료되면 실제 해당 명령을 수행해서 Redis 메모리에 내용을 변경
...
- 복구 시 저장된 모든 연산을 다시 실행하므로 백업 및 복구 속도가 느리다.
    - (ex) set 명령이 Key가 같고 Value가 다른 연산이 여러번 수행된다면, 메모리에는 마지막 값만 남아있지만, AOF에는 모든 기록이 남아있다.
